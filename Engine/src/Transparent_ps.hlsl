...
PixelOut main(PixelIn input)
{
    ...
    const uint startOffset = t_lightGridTrans[tileIndex].x;
    const uint lightCount = t_lightGridTrans[tileIndex].y;

    [loop]
    for (int i = 0; i < lightCount; i++)
    {
        const uint lightIndex = sb_lightIndexListTrans[startOffset + i];
    ...
    */
    ...
    output.color = float4(color, c_transparency);
    return output;
}
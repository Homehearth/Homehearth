...
void BloomPass::Setup()
{
    ...
    m_screenSpaceQuad.Create(D3D11Core::Get().Device());
    m_screenSpaceQuad.SetData(D3D11Core::Get().DeviceContext(), screenQuads);
    ...
    texDesc.Height /= 2;
    texDesc.Width /= 2;
    D3D11Core::Get().Device()->CreateTexture2D(&texDesc, nullptr, m_quarterSize.GetAddressOf());
    D3D11Core::Get().Device()->CreateShaderResourceView(m_quarterSize.Get(), NULL, m_quarterSizeView.GetAddressOf());

    texDesc.Height /= 2;
    texDesc.Width /= 2;
    D3D11Core::Get().Device()->CreateTexture2D(&texDesc, nullptr, m_smolSize.GetAddressOf());
    D3D11Core::Get().Device()->CreateShaderResourceView(m_smolSize.Get(), NULL, m_smolSizeView.GetAddressOf());
}

...
bool RTexture::StandardSetup(unsigned char* image, const UINT& width, const UINT& height)
{
	...
	hr = D3D11Core::Get().Device()->CreateShaderResourceView(m_texture.Get(), 0, &m_shaderView);
	if (FAILED(hr))
	{
#ifdef _DEBUG
...
bool RTexture::GenerateMipMaps(unsigned char* image, const UINT& width, const UINT& height)
{
	...
	HRESULT hr = D3D11Core::Get().Device()->CreateTexture2D(&textureDesc, nullptr, &m_texture);
	if (FAILED(hr))
	{
#ifdef _DEBUG
		LOG_WARNING("[Texture2D] Failed to create Texture2D!");
	...
	D3D11Core::Get().DeviceContext()->UpdateSubresource(m_texture.Get(), 0u, nullptr, (void*)image, mempitch, 0u);
	...
	hr = D3D11Core::Get().Device()->CreateShaderResourceView(m_texture.Get(), &srvDesc, &m_shaderView);
	if (FAILED(hr))
	{
#ifdef _DEBUG
		LOG_WARNING("[Texture2D] Failed to create ShaderResourceView!");
